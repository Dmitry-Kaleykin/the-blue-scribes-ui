...
export async function registerApiRoutes(
  ...
  app.post('/api/profiles', async (request) => {
    ...
    const dimensions = input.detectDimensions === true
      ? (await profiles.inspectLmStudioEmbeddingModel(
          input.model,
          input.baseUrl,
          input.embeddingSuffix,
        )).dimensions
      : input.dimensions
    ...
    return profiles.set({
      ...
      embedding: {
        ...
        ...(input.maximumInputs === undefined
          ? {}
          : { maximumInputs: input.maximumInputs }),
        ...(input.embeddingSuffix === undefined
          ? {}
          : { embeddingSuffix: input.embeddingSuffix }),
      },
      ...(input.rerankingModel === undefined
        ...
        : {
            reranking: {
              ...
              ...(input.baseUrl === undefined ? {} : { baseUrl: input.baseUrl }),
              ...(input.rerankingInstruction === undefined
                ? {}
                : { instruction: input.rerankingInstruction }),
            },
          }),
    })
  })
  ...
  app.post('/api/projects/:id/reindex', async (request, reply) => {
    ...
    const project = (await listIndexedProjects()).find(
      ({ projectIdentifier }) => projectIdentifier === id,
    )
  ...
  app.post('/api/projects/:id/search', async (request) => {
    ...
    const profile = input.profile ?? (
      recipe?.provider.type === 'profile' ? recipe.provider.profile : undefined
    )
    return search.search({
      ...
      ...(input.reranking === undefined && input.rerankCandidates === undefined
        ...
        : {
            reranking: {
              ...(input.reranking === undefined
                ? {}
                : { enabled: input.reranking }),
  ...
  async function projectSummaries(): Promise<ProjectSummary[]> {
    ...
    return Promise.all(projects.map(async (project): Promise<ProjectSummary> => {
      try {
...
function profileInput(value: unknown): ProfileInput {
  ...
  return {
    ...
    ...(boolean(body.detectDimensions, 'detectDimensions', { optional: true }) === undefined
      ? {}
    ...
    ...(integer(body.maximumInputs, 'maximumInputs', {
      ...
      ? {}
      : { maximumInputs: integer(body.maximumInputs, 'maximumInputs', { minimum: 1 })! }),
    ...(typeof body.embeddingSuffix === 'string'
      ? { embeddingSuffix: body.embeddingSuffix }
      : {}),
    ...(text(body.rerankingModel, 'rerankingModel', { optional: true }) === undefined
      ? {}
      : { rerankingModel: text(body.rerankingModel, 'rerankingModel')! }),
    ...(text(body.rerankingInstruction, 'rerankingInstruction', { optional: true }) === undefined
      ? {}
...
function indexProjectInput(value: unknown): IndexProjectInput {
  ...
  return {
    ...
    ...(text(body.target, 'target', { optional: true }) === undefined
      ? {}
      : { target: text(body.target, 'target')! }),
    keepReplacedBuilds: integer(
      body.keepReplacedBuilds ?? 1,
      'keepReplacedBuilds',
      { minimum: 0 },
    )!,
    ...
    ...(integer(body.maximumChunkSize, 'maximumChunkSize', {
      ...
      ? {}
      : {
          maximumChunkSize: integer(
            body.maximumChunkSize,
            'maximumChunkSize',
            { minimum: 1 },
          )!,
        }),
    ...(stringList(body.include, 'include') === undefined
      ? {}
      : { include: stringList(body.include, 'include')! }),
    ...(stringList(body.exclude, 'exclude') === undefined
      ? {}
      : { exclude: stringList(body.exclude, 'exclude')! }),
  }
}
...
function searchInput(value: unknown): SearchInput {
  ...
  return {
    ...
    ...(integer(body.rerankCandidates, 'rerankCandidates', {
      ...
      ? {}
      : {
          rerankCandidates: integer(
            body.rerankCandidates,
            'rerankCandidates',
            { minimum: 1 },
          )!,
        }),
