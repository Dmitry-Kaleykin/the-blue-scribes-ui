...
async function save(): Promise<void> {
  ...
  try {
    const input: ProfileInput = {
      ...
      ...(form.rerankingModel.trim()
        ? { rerankingModel: form.rerankingModel.trim() }
        : {}),
      ...(form.rerankingInstruction.trim()
        ? { rerankingInstruction: form.rerankingInstruction.trim() }
        : {}),
    }
    ...
    <form class="form-stack" @submit.prevent="save">
      <div class="form-grid form-grid--two">
        <label class="field">
          ...
          <input v-model="form.name" required placeholder="local-qwen" :disabled="profile !== undefined">
          <small v-if="profile">Profile names are stable identifiers.</small>
        </label>
        <label class="field">
          <span>LM Studio base URL</span>
          <input v-model="form.baseUrl" placeholder="http://127.0.0.1:1234/v1">
        </label>
      </div>
      ...
      <div class="form-section">
        <div class="section-heading section-heading--compact">
          ...
          <button class="button button--secondary button--small" type="button" :disabled="loadingModels" @click="discoverModels">
            <LoaderCircle v-if="loadingModels" class="spin" :size="15" />
        ...
        <div class="form-grid form-grid--two">
          <label class="field">
            ...
            <input v-model="form.model" required list="lm-models" placeholder="text-embedding-qwen3-embedding-0.6b">
          ...
          <label class="field">
            ...
            <input v-model.number="form.maximumInputs" required min="1" type="number">
          </label>
        </div>
        <div class="form-grid form-grid--two">
          <label class="check-field">
            <input v-model="form.detectDimensions" type="checkbox">
            <span>
          ...
          <label class="field">
            ...
            <input v-model.number="form.dimensions" :disabled="form.detectDimensions" min="1" type="number">
          </label>
        </div>
        <label class="field">
          <span>Embedding suffix <em>optional</em></span>
          <input v-model="form.embeddingSuffix" placeholder="<|endoftext|>">
      ...
      <div class="form-section">
        ...
        <div class="form-grid form-grid--two">
          <label class="field">
            ...
            <input v-model="form.rerankingModel" list="lm-models" placeholder="qwen3-reranker-0.6b">
          </label>
          <label class="field">
            <span>Instruction</span>
            <input v-model="form.rerankingInstruction" placeholder="Retrieve relevant source code">
          </label>
        </div>
      </div>
      ...
      <footer class="form-actions">
        <button v-if="canTest" class="button button--secondary" type="button" :disabled="testing" @click="testSavedProfile">
          <LoaderCircle v-if="testing" class="spin" :size="16" />
