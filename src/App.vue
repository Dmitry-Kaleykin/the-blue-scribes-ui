...
const selectedProject = computed(() => data.value.projects.find(
  ({ projectIdentifier }) => projectIdentifier === selectedProjectId.value,
))
const activeJobCount = computed(() => data.value.jobs.filter(
...
function watchJob(id: string): void {
  ...
  subscriptions.set(id, subscribeToJob(id, (event) => {
...
async function testProfile(profile: ProviderProfile): Promise<void> {
  ...
  try {
    ...
    const reranking = result.reranking
      ? ` Reranker score: ${result.reranking.score.toFixed(3)}.`
      : ''
    showToast(
      'success',
      `${result.embedding.model} returned ${result.embedding.dimensions} dimensions.${reranking}`,
    )
  } catch (reason: unknown) {
    showFailure(reason)
  }
...
async function removeProfile(profile: ProviderProfile): Promise<void> {
  if (!window.confirm(`Delete provider profile "${profile.name}"? Existing recipes that refer to it will need another profile.`)) return
  try {
...
function showToast(tone: 'success' | 'danger', message: string): void {
  ...
  if (toastTimer) clearTimeout(toastTimer)
  toastTimer = setTimeout(() => { toast.value = undefined }, 5000)
}
</script>
  ...
  <div class="app-shell">
    <aside class="sidebar" :class="{ 'sidebar--open': mobileMenu }">
      <div class="brand">
        ...
        <button class="icon-button sidebar__close" type="button" aria-label="Close menu" @click="mobileMenu = false">
          <X :size="19" />
        </button>
      </div>
      ...
      <nav class="nav-list" aria-label="Primary navigation">
        ...
        <button :class="{ active: activeView === 'search' }" type="button" @click="openSearch()">
          <Search :size="19" /> Search
        </button>
        <button :class="{ active: activeView === 'profiles' }" type="button" @click="navigate('profiles')">
      ...
      <div class="sidebar__section">
        ...
        <button
          ...
          class="project-link"
          :class="{ active: activeView === 'project' && selectedProjectId === project.projectIdentifier }"
          type="button"
        ...
        <span v-if="data.projects.length === 0" class="sidebar__empty">No indexes yet</span>
      </div>

      <div class="sidebar__bottom">
        <button class="sidebar-index" type="button" :disabled="data.profiles.length === 0" @click="indexModal = true">
    ...
    <section class="workspace">
      <header class="topbar">
        <button class="icon-button mobile-menu" type="button" aria-label="Open menu" @click="mobileMenu = true">
          <Menu :size="20" />
        </button>
        <div class="topbar__status">
          <span v-if="activeJobCount > 0"><LoaderCircle class="spin" :size="15" /> {{ activeJobCount }} indexing</span>
          <span v-else><i /> Ready</span>
        </div>
        <button class="icon-button" type="button" aria-label="Refresh workspace" @click="load">
          <Settings :size="18" />
        </button>
      </header>
    ...
    <div v-if="toast" class="toast" :class="`toast--${toast.tone}`" role="status">
      <span />
      {{ toast.message }}
      <button type="button" aria-label="Dismiss notification" @click="toast = undefined"><X :size="16" /></button>
    </div>
  </div>
</template>
