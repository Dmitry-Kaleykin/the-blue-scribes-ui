...
const activeJobs = computed(() => props.jobs.filter(
  ({ status }) => status === 'running' || status === 'queued',
))
const recentJobs = computed(() => props.jobs.filter(
  ({ status }) => status !== 'running' && status !== 'queued',
).slice(0, 3))
const readyBuilds = computed(() => props.projects.reduce(
  (total, project) => total + project.buildsByStatus.ready,
  0,
))
const storage = computed(() => props.projects.reduce(
  (total, project) => total + project.databaseBytes,
  0,
))
...
<template>
  <main class="page">
    <header class="page-header page-header--split">
      ...
      <button class="button button--primary" type="button" @click="emit('index')">
        <Plus :size="17" />
        Index project
      </button>
    </header>
    ...
    <section class="metric-grid" aria-label="Workspace summary">
      <article class="metric-card">
        ...
        <div><strong>{{ projects.length }}</strong><span>Indexed projects</span></div>
      </article>
      <article class="metric-card">
        <span class="metric-card__icon metric-card__icon--teal"><Layers3 :size="20" /></span>
        <div><strong>{{ readyBuilds }}</strong><span>Ready builds</span></div>
      </article>
      <article class="metric-card">
        <span class="metric-card__icon metric-card__icon--violet"><Server :size="20" /></span>
        <div><strong>{{ profiles.length }}</strong><span>Provider profiles</span></div>
      </article>
      <article class="metric-card">
        <span class="metric-card__icon metric-card__icon--sand"><Database :size="20" /></span>
        <div><strong>{{ formatBytes(storage) }}</strong><span>Local index storage</span></div>
      </article>
    </section>

    <section v-if="activeJobs.length > 0 || recentJobs.length > 0" class="content-section">
    ...
    <section class="content-section">
      ...
      <div v-if="projects.length > 0" class="project-table">
        ...
        <article v-for="project in projects" :key="project.projectIdentifier" class="project-row">
          <button class="project-row__identity" type="button" @click="emit('project', project)">
          ...
          <div class="project-row__build">
            ...
            <small v-if="project.latestReadyBuild">
              {{ project.latestReadyBuild.dimensions }} dimensions
            </small>
          </div>
          <span class="mono-muted">{{ formatBytes(project.databaseBytes) }}</span>
          <div class="project-row__actions">
            <button class="button button--secondary button--small" type="button" @click="emit('search', project)">
              <Search :size="15" /> Search
            </button>
            <button class="icon-button" type="button" aria-label="Open project" @click="emit('project', project)">
              <ArrowRight :size="18" />
            </button>
          </div>
        </article>
      ...
      <div v-else class="empty-state">
        ...
        <div class="button-row">
          <button v-if="profiles.length === 0" class="button button--secondary" type="button" @click="emit('profiles')">
            <Server :size="16" /> Set up a profile
          </button>
          <button class="button button--primary" type="button" :disabled="profiles.length === 0" @click="emit('index')">
    ...
    <section class="quick-actions">
      <button class="quick-action" type="button" @click="emit('index')">
        <span><RefreshCw :size="19" /></span>
        <strong>Start an index</strong>
        <small>Save a recipe and monitor every phase.</small>
      </button>
      <button class="quick-action" type="button" @click="emit('profiles')">
        <span><Server :size="19" /></span>
        <strong>Manage providers</strong>
        <small>Discover and diagnose LM Studio models.</small>
      </button>
      <button class="quick-action" type="button" :disabled="projects.length === 0" @click="projects[0] && emit('search', projects[0])">
