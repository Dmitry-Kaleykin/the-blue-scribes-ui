...
const selectedProject = computed(() => props.projects.find(
  ({ projectIdentifier }) => projectIdentifier === form.project,
))
const effectiveProfile = computed(() => {
  const recipe = selectedProject.value?.recipe
  const name = form.profile || (
    recipe?.provider.type === 'profile' ? recipe.provider.profile : ''
  )
...
<template>
  <main class="page">
    <header class="page-header">
      ...
      <p class="page-lead">Ask a question, compare semantic and reranking scores, and inspect the exact source context.</p>
    </header>
    ...
    <section class="search-panel">
      <form @submit.prevent="submit">
        <div class="search-panel__selectors">
          <label class="field">
            ...
            <select v-model="form.project" required>
              <option disabled value="">Choose an indexed project</option>
              <option v-for="project in projects" :key="project.projectIdentifier" :value="project.projectIdentifier">
        ...
        <label class="search-box">
          ...
          />
          <button class="button button--primary" type="submit" :disabled="searching || !form.project">
            <LoaderCircle v-if="searching" class="spin" :size="17" />
            <Search v-else :size="17" />
            {{ searching ? 'Searching…' : 'Search' }}
          </button>
        </label>
        <div class="search-panel__footer">
          <span v-if="selectedProject?.active" class="muted">
            Searching {{ selectedProject.active.target ?? selectedProject.active.type }}
          </span>
          <span v-else />
          <button class="disclosure" type="button" @click="advanced = !advanced">
        ...
        <div v-if="advanced" class="advanced-search">
          <label class="field">
            <span>Results</span>
            <input v-model.number="form.limit" min="1" type="number">
          </label>
          <label class="field">
            <span>Language filter</span>
            <input v-model="form.language" placeholder="typescript">
          </label>
          <label class="field">
            <span>Candidate pool</span>
            <input v-model.number="form.rerankCandidates" :disabled="!form.reranking" min="1" type="number">
          </label>
          <label class="field">
            <span>Context before</span>
            <input v-model.number="form.contextBefore" min="0" type="number">
          </label>
          <label class="field">
            <span>Context after</span>
            <input v-model.number="form.contextAfter" min="0" type="number">
          </label>
          <label class="field">
            <span>Context characters</span>
            <input v-model.number="form.contextCharacters" min="1" type="number">
          </label>
          <label class="check-field check-field--compact">
            <input v-model="form.reranking" :disabled="!rerankingAvailable" type="checkbox">
            <span>
              <strong>Local reranking</strong>
              <small>{{ rerankingAvailable ? `Use ${effectiveProfile?.reranking?.model}.` : 'Choose a profile with a reranker.' }}</small>
            </span>
          </label>
        </div>
      </form>
    ...
    <p v-if="error" class="feedback feedback--danger">{{ error }}</p>

    <section v-if="response" class="content-section results-section">
      ...
      <div v-if="response.results.length > 0" class="result-list">
        <article v-for="(result, index) in response.results" :key="`${result.path}:${result.range.startOffset}`" class="search-result">
          <header>
            <span class="result-rank">{{ index + 1 }}</span>
            <div class="search-result__identity">
              <h3><FileCode2 :size="17" /> {{ result.path }}</h3>
              <p>Lines {{ result.range.startLine }}–{{ result.range.endLine }} · {{ result.language }}</p>
            </div>
            <div class="score-set">
              <span><small>Final</small><strong>{{ score(result.score) }}</strong></span>
              <span><small>Semantic</small><strong>{{ score(result.semanticScore) }}</strong></span>
              <span v-if="result.rerankScore !== undefined"><small>Rerank</small><strong>{{ score(result.rerankScore) }}</strong></span>
            </div>
          </header>
          <pre><code>{{ result.content }}</code></pre>
          <footer v-if="result.context && (result.context.before.length > 0 || result.context.after.length > 0)">
            <button class="button button--ghost button--small" type="button" @click="toggleResult(index)">
              <ChevronUp v-if="expandedResults.has(index)" :size="15" />
              <ChevronDown v-else :size="15" />
              {{ expandedResults.has(index) ? 'Hide neighboring chunks' : 'Show neighboring chunks' }}
            </button>
            <div v-if="expandedResults.has(index)" class="context-chunks">
              <article v-for="chunk in result.context.before" :key="chunk.chunkId">
                <span>Before · lines {{ chunk.range.startLine }}–{{ chunk.range.endLine }}</span>
                <pre><code>{{ chunk.content }}</code></pre>
              </article>
              <article v-for="chunk in result.context.after" :key="chunk.chunkId">
